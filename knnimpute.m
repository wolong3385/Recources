...
noNans = sum(nanVals,2) == 0; % check which row don't have missing data
...
% distances = pdist(dataNoNans',metric,distargs{:}); %original distances
...
dists(1,:) = 0;
equalDists = [zeros(1,size(dists,2));(diff(dists)== 0);]; %diff() means later row mines previous row. http://zhidao.baidu.com/link?url=jAgn3DPKoHNHck3cxSIJCuEFA0i2Rj_z7KJ3Yxe9ytE0AjunxiGswCmUDrAdHfBmxcZe9xn4kffmvhmmjB2OUq
...
for count = 1:numel(rows)
    % check that we don't have a row of nans, if all the data in the row is
    % missing, imputation make no sense
    ...
    for nearest = 2:size(ndx,1)-K+1
        ...
        % calculation.
%         q=nearest
%         qq=equalDists(nearest+K-1:end,cols(count))
%         qqq=find(equalDists(nearest+K-1:end,cols(count))==0,1)
        ...
        if useWMean
            if ~userWeights
                ...
                if L > 0
                    weights = [uWeights(1:end-1); repmat(uWeights(end)/L,L+1,1)];
                else
                    weights = uWeights;
                end
            end
